...
class PExceptions implements Exception {
  ...
  factory PExceptions.fromCode(String code) {
    switch (code) {
      ...
      case 'user-not-found':
        return const PExceptions('Invalid login details. User not found.');
      case 'wrong-password':
        return const PExceptions(
      ...
      case 'too-many-requests':
        return const PExceptions('Too many requests. Please try again later.');
      case 'invalid-argument':
        return const PExceptions(
            'Invalid argument provided to the authentication method.');
      case 'invalid-password':
        return const PExceptions('Incorrect password. Please try again.');
      case 'invalid-phone-number':
        return const PExceptions('The provided phone number is invalid.');
      ...
      case 'sign_in_failed':
        return const PExceptions('Sign-in failed. Please try again.');
      case 'network-request-failed':
        return const PExceptions(
            'Network request failed. Please check your internet connection.');
      case 'internal-error':
        return const PExceptions('Internal error. Please try again later.');
      ...
      case 'quota-exceeded':
        return const PExceptions('Quota exceeded. Please try again later.');
      default:
        return const PExceptions();
    }
  }
